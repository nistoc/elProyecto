...
public sealed class InMemoryNodeStore : INodeModel, INodeQuery
{
    ...
    public void StartNode(string nodeId)
    {
        ...
        {
            node.Status = RefineJobState.Running;
            node.StartedAt ??= now;
            node.UpdatedAt = now;
        }
    }

    public void CompleteNode(string nodeId, RefineJobState status, DateTimeOffset? completedAt = null, string? errorMessage = null)
    {
