...
- **Примечание:** При каждом запуске папка автоматически очищается

#### intermediate_dir
Папка для промежуточных результатов.
- **По умолчанию:** `intermediate_fixes`
- **Очистка:** Содержимое папки автоматически очищается перед каждым запуском
...
### Об изменении количества строк

GPT может **объединять последовательные реплики одного спикера** для улучшения читаемости. Это нормальное поведение.

**Пример:**

До (3 строки):
```
- 72.12 speaker_0: "vale"
- 72.50 speaker_0: "por favor"
- 73.00 speaker_0: "es que"
```

После (1 строка):
```
- 72.12 speaker_0: "vale, por favor, es que"
```

Вы увидите в логах:
```
[INFO] GPT merged lines: 30 → 27 (Δ-3)
```

Это **не ошибка** - GPT улучшает структуру текста, сохраняя смысл.
