# 🎯 Agent03 - Modular Transcription System
...
- 🎯 Система транскрибирует с каждым языком + автоопределение
- 🇪🇸🇷🇺 По умолчанию: испанский и русский (`["es", "ru"]`)
- 📊 Все результаты сохраняются: `text-es`, `text-ru`, `text-null`
- ⚠️ 3 API вызова на сегмент (es + ru + null)
...
**Исходный код:**
```
agent03/                        # Корневая папка проекта
...
agent03 --config config/default.json
```
...
- `languages` - список языков для транскрипции
- Система транскрибирует с каждым языком + автоопределение
- Все результаты сохраняются: `text-es`, `text-ru`, `text-null`
- По умолчанию: `["es", "ru"]` → 3 API вызова на сегмент
...
```python
# Диаризация (v3.0)
from agent03.infrastructure.audio import AudioDiarizer
...
# Chunking (v2.x)
from agent03.infrastructure.audio import AudioChunker
...
# Транскрипция
from agent03.services import OpenAITranscriptionClient
...
# Кеш
from agent03.infrastructure.cache import CacheManager
